...
fn test_legacy() {
    ...
    let json = decoder::run(raw_transaction_hex.to_string()).unwrap();
    let expected = fs::read_to_string("tests/test_transaction_legacy.json").unwrap();
    let actual_v: Value = serde_json::from_str(&json).unwrap();
    let expected_v: Value = serde_json::from_str(&expected).unwrap();
    assert_eq!(expected_v, actual_v, "JSON values differ");
}
